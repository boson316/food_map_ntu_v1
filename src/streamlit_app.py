...
def _render_food_group_selector() -> list[str]:
    """側欄分類：分組 multiselect，另可勾選「其他」。"""
    selected: list[str] = []
    with st.expander("美食分類（分組）", expanded=True):
        st.caption("未選任何項 = 顯示全部；轉盤有選時自動併入「其他」。")
        for label, groups in FOOD_GROUP_SIDEBAR_GROUPS:
            picked = st.multiselect(label, options=list(groups), default=[], key=f"fg_{label}")
            selected.extend(picked)
        if st.checkbox("篩選「其他」分類", value=False, key="fg_include_other"):
            selected.append(FOOD_GROUP_OTHER)
    return selected
...
def _load_wheel_pinned_ids() -> set[str]:
    ...
    try:
        ...
        return set()
    ids = raw.get("place_ids")
    if not isinstance(ids, list):
        return set()
    return {str(place_id) for place_id in ids if str(place_id).strip()}


def _default_campus() -> tuple[float, float]:
    lat = float(os.environ.get("CAMPUS_LAT", "25.0173"))
    lon = float(os.environ.get("CAMPUS_LON", "121.5397"))
    return lat, lon
...
def _geocode_place_name(query: str) -> tuple[float, float] | None:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "food-map-ntu-v1-streamlit/1.0 (school project demo)"},
    )
    try:
    ...
    return pd.DataFrame(
        {
            ...
            "營業狀態": df["open_status_display"],
            "Google 評分": df["rating"],
            "評論數": df["review_count"],
            "平均消費": df["avg_spend_display"],
            "美食分類": df["food_group_display"],
...
box-shadow:0 1px 3px rgba(60,64,67,.15);margin-bottom:0.75rem;">
  ...
  <div style="margin-top:0.35rem;color:#5f6368;font-size:0.92rem;">
    ...
    （{int(row["review_count"])} 則評論）　{row["open_status_display"]}　平均消費 {row["avg_spend_display"]}　距離 {row["distance_display"]}
  </div>
...
def _render_map(
    *,
    ...
    center_layer = pdk.Layer(
        "ScatterplotLayer",
        data=pd.DataFrame({"lat": [center_lat], "lon": [center_lon], "name": ["台大校本部"]}),
        get_position=["lon", "lat"],
        ...
        st.dataframe(
            wheel_df[
                ...
                columns={
                    ...
                    "open_status_display": "營業狀態",
                    "food_group_display": "美食分類",
                    "distance_display": "距離",
                    "rating": "Google 評分",
                    "review_count": "評論數",
                }
...
def run() -> None:
    ...
    st.title("國立臺灣大學 校園美食地圖")
    st.caption(
        "中心點預設：台大校本部（25.0173, 121.5397）。"
    ...
    with st.sidebar:
        ...
        with st.expander("用地名設定中心（可選）", expanded=False):
            ...
            place_query = st.text_input("輸入地名", placeholder="例：國立臺灣大學")
            if st.button("查詢並設為中心", key="geocode_btn"):
                coords = _geocode_place_name(place_query)
                if coords is None:
                    st.error("找不到地點，請換關鍵字（可加「台北」或「公館」）。")
                else:
        ...
        radius = st.slider("搜尋半徑（公里）", min_value=0.3, max_value=5.0, value=1.0, step=0.1)
        ...
        if budget_filter:
            max_avg_spend_ntd = st.number_input(
                "人均上限（新台幣）",
                min_value=50,
                max_value=500,
                value=100,
                step=10,
                help="overlay avg_spend_ntd 或 Google price_level 1 皆可通過",
            )
        include_unknown_price = st.checkbox("預算篩選時含未標價位", value=True)
        ...
        limit = st.slider("最多顯示筆數", min_value=5, max_value=300, value=300, step=5)
    ...
    rows = service.rank_nearby(
        ...
        sort_by=sort_mode,
        limit=int(limit),
        max_price_level=1 if budget_filter else None,
        max_avg_spend_ntd=int(max_avg_spend_ntd) if max_avg_spend_ntd is not None else None,
        include_unknown_price=include_unknown_price,
        food_groups=selected_food_groups or None,
    )
    ...
    with tab_map:
        ...
        st.subheader("地圖")
        st.caption("淺色街道圖 · 🔵 台大校本部 · 🔴 餐廳（點狀標記）· 🟡 你選的店")
        if focus_row is not None:
