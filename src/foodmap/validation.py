...
def validate_restaurant_record(raw: Any, *, index: int) -> None:
    ...
    if raw.get("business_status") is not None and not str(raw["business_status"]).strip():
        raise ValueError(f"{prefix}.business_status must be a non-empty string when set")

    food_groups = raw.get("food_groups")
    if food_groups is not None:
        if not isinstance(food_groups, list) or not all(isinstance(g, str) for g in food_groups):
            raise ValueError(f"{prefix}.food_groups must be a list of strings")

    opening_hours = raw.get("regular_opening_hours")
    if opening_hours is not None:
        if not isinstance(opening_hours, dict):
            raise ValueError(f"{prefix}.regular_opening_hours must be an object")
        periods = opening_hours.get("periods")
        if periods is not None and not isinstance(periods, list):
            raise ValueError(f"{prefix}.regular_opening_hours.periods must be a list")
