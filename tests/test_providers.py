...
def test_google_post_json_retries_on_timeout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("GOOGLE_PLACES_MAX_RETRIES", "2")
    monkeypatch.setenv("GOOGLE_PLACES_TIMEOUT_S", "5")
    ...
    def fake_urlopen(req, timeout=0):  # noqa: ARG001
        ...
        payload = MagicMock()
        payload.read.return_value = json.dumps({"places": []}).encode("utf-8")
        payload.__enter__ = lambda self: payload
        payload.__exit__ = MagicMock(return_value=False)
        return payload

    with patch("urllib.request.urlopen", side_effect=fake_urlopen):
        result = provider._post_json({"includedTypes": ["restaurant"]})
